...
public class UBRec {
	...
	public UBRec(Config conf, double[] parameters)  {
		try
		{
			this.conf=conf;
	...
	private void recommend() throws Exception {
		...
		for(String student:candidates.keySet()) {
			...
			for(String candidateItem:candidates.get(student)) {
				...
				// use the item utility in view of the students to rank the items
				double score = itemUtility_student;				
